...
function sendMoney(options) {
  ...
  //   console.log(err || result);
  // });
  var trs = acmeJS.transaction.createTransaction(
    options.to,
...
function registerDelegate(options) {
  ...
  //   console.log(err || result);
  // });
  var trs = acmeJS.delegate.createDelegate(
    options.secret,
    options.username,
    options.secondSecret
  );
...
function vote(secret, publicKeys, op, secondSecret) {
  ...
  var trs = acmeJS.vote.createVote(
    secret,
    votes,
    secondSecret
  );
...
function setSecondSecret(options) {
  var trs = acmeJS.signature.createSignature(options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.success);
  });
}
...
function registerDapp(options) {
  ...
  var trs = acmeJS.dapp.createDapp(options.secret, options.secondSecret, dapp);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.success);
